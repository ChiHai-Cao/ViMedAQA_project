""" 
Complete Vietnamese Document Retrieval and Reranking Evaluation System 
Author: AI Assistant 
Date: 2025 

This module provides a complete pipeline for: 
1. Embedding-based document retrieval 
...
# ============================================================================ 
# PART 1: EMBEDDING-BASED RETRIEVAL SYSTEM 
# ============================================================================ 
...
class VietnameseDataset: 
    ...
    def __init__(self, train_path: str, articles_path: str): 
        ...
        (self.questions, self.true_keys, self.documents, self.document_urls, self.key2docidx) = self._load_data() 
...
class VietnameseEmbeddingModel: 
    ...
    def encode_texts(self, texts: List[str], batch_size: int = 16, show_progress: bool = True) -> np.ndarray: 
        """ 
...
class CSVLogger: 
    ...
    def _initialize_csv_files(self) -> None: 
        ...
        with open(self.metrics_csv, 'w', newline='', encoding='utf-8') as f: 
            writer = csv.writer(f) 
            writer.writerow([ 
                'timestamp', 'model_name', 'max_seq_length', 'num_questions', 'num_documents', 
                'valid_samples', 'total_samples', 'MRR', 'Recall@1', 'Recall@3', 'Recall@5', 'Recall@10' 
            ]) 
    ...
    def log_metrics(self, model_name: str, max_seq_length: int, dataset_stats: Dict, metrics: Dict) -> None: 
        """Log overall metrics to CSV.""" 
        with open(self.metrics_csv, 'a', newline='', encoding='utf-8') as f: 
            writer = csv.writer(f) 
            writer.writerow([ 
                datetime.now().isoformat(), model_name, max_seq_length, 
                dataset_stats.get('num_questions', 0), dataset_stats.get('num_documents', 0), 
                metrics.get('Valid_Samples', 0), metrics.get('Total_Samples', 0), 
                metrics.get('MRR', 0), metrics.get('Recall@1', 0), metrics.get('Recall@3', 0), 
    ...
    def log_search_results(self, sample_id: int, question: str, retrieved_docs: List[Dict]) -> None: 
        ...
        with open(self.search_logs_csv, 'a', newline='', encoding='utf-8') as f: 
            ...
            for doc in retrieved_docs: 
                writer.writerow([ 
                    sample_id, question_length, search_timestamp, doc['rank'], 
                    doc['document_index'], doc['similarity_score'], doc['document_url'], doc['is_relevant'] 
                ]) 
...
class EmbeddingEvaluator: 
    ...
    def run_evaluation( 
        ...
        csv_base_filename: str = "vietnamese_embedding_eval", 
        top10_output_file: str = "top10_relevant_documents.json" 
        ...
        Args: 
            ...
            top10_output_file: Output file path for top-10 documents 
        ...
        self.doc_embeddings = self.model.encode_texts( 
            self.dataset.documents, batch_size=batch_size 
        ) 
        ...
        self.q_embeddings = self.model.encode_texts( 
            self.dataset.questions, batch_size=batch_size 
        ) 

        # Calculate metrics 
        print("\n5. Calculating metrics...") 
        metrics, sample_results = EvaluationMetrics.calculate_metrics( 
            self.index, self.q_embeddings, self.dataset.questions, self.dataset.true_keys, 
            self.dataset.document_urls, self.dataset.key2docidx, k_values, self.csv_logger 
        ) 
        ...
        if self.csv_logger: 
            ...
            self.csv_logger.log_metrics( 
                self.model.model_name, self.model.max_seq_length, 
                self.dataset.get_stats(), metrics 
            ) 
            self.csv_logger.log_detailed_results(sample_results) 

        # Save top-10 documents JSON 
        print(f"\n{'7' if self.csv_logger else '6'}. Saving top-10 relevant documents...") 
        top10_path = self._save_top10_documents(sample_results, top10_output_file) 

        # Save JSON results 
        print(f"\n{'8' if self.csv_logger else '7'}. Saving JSON results...") 
        output_path = self._save_results( 
            self.model.model_name, metrics, sample_results, output_file 
        ) 

        # Display results 
        csv_paths = self.csv_logger.get_csv_paths() if self.csv_logger else {} 
        self._display_results( 
            self.model.model_name, metrics, sample_results, 
            output_path, csv_paths, top10_path 
        ) 
        ...
        return { 
            ...
            "top10_file": top10_path, 
            "csv_files": csv_paths 
        } 

    def _save_top10_documents(self, sample_results: List[Dict], output_file: str) -> str: 
        """Save top-10 relevant documents for each question to a JSON file.""" 
        ...
        for sample in sample_results: 
            top10_data.append({ 
                ...
                "top_10_documents": sample["top_10_relevant_documents"] 
            }) 

        output_path = Path(output_file) 
        with open(output_path, "w", encoding="utf-8") as f: 
            json.dump(top10_data, f, ensure_ascii=False, indent=2, default=convert_numpy_types) 

        return str(output_path) 

    def _save_results( 
        self, model_name: str, metrics: Dict[str, float], 
        sample_results: List[Dict], output_file: str 
    ) -> str: 
    ...
    def _display_results( 
        self, model_name: str, metrics: Dict[str, float], sample_results: List[Dict], 
        output_file: str, csv_files: Dict[str, str] = {}, top10_file: str = "" 
    ) -> None: 
        ...
        if top10_file: 
            print(f"Top-10 documents saved to: {top10_file}") 
...
class RetrievalResultsLoader: 
    ...
    def get_question_documents(self, question_id: int) -> Tuple[str, List[Dict]]: 
        """Get question and its retrieved documents.""" 
        if question_id >= len(self.retrieval_results): 
            raise IndexError(f"Question ID {question_id} out of range") 

        result = self.retrieval_results[question_id] 
        return result['question'], result['top_10_documents'] 
...
class RerankerModel: 
    ...
    def load_model(self) -> None: 
        ...
        try: 
            ...
            self._is_loaded = True 
            logging.info(f"Reranker model {self.model_name} loaded successfully on {self.device}") 
        except Exception as e: 
    ...
    def rerank(self, query: str, documents: List[str], batch_size: int = 32) -> np.ndarray: 
        ...
        Returns: 
            numpy array of reranking scores 
        """ 
        self._ensure_model_loaded() 
        if not documents: 
            return np.array([]) 
        ...
        scores = self.model.predict( 
            query_doc_pairs, batch_size=batch_size, show_progress_bar=False 
        ) 
...
class RerankerCSVLogger: 
    ...
    def _initialize_csv_files(self) -> None: 
        ...
        with open(self.rerank_logs_csv, 'w', newline='', encoding='utf-8') as f: 
            writer = csv.writer(f) 
            writer.writerow([ 
                'sample_id', 'question_length', 'rerank_timestamp', 'original_rank', 'new_rank', 
                'retrieval_score', 'rerank_score', 'document_url', 'is_relevant' 
            ]) 
        ...
        with open(self.comparison_csv, 'w', newline='', encoding='utf-8') as f: 
            writer = csv.writer(f) 
            writer.writerow([ 
                'sample_id', 'question', 'retrieval_rank', 'rerank_rank', 'rank_change', 
                'retrieval_mrr', 'rerank_mrr' 
            ]) 

    def log_metrics(self, reranker_model: str, retriever_model: str, num_questions: int, metrics: Dict) -> None: 
        """Log overall metrics to CSV.""" 
        with open(self.metrics_csv, 'a', newline='', encoding='utf-8') as f: 
            writer = csv.writer(f) 
            writer.writerow([ 
                datetime.now().isoformat(), reranker_model, retriever_model, num_questions, 
                metrics.get('Valid_Samples', 0), metrics.get('Total_Samples', 0), 
                metrics.get('MRR', 0), metrics.get('Recall@1', 0), metrics.get('Recall@3', 0), 
                metrics.get('Recall@5', 0), metrics.get('Recall@10', 0), 
    ...
    def log_detailed_results(self, sample_results: List[Dict]) -> None: 
        ...
        with open(self.detailed_csv, 'a', newline='', encoding='utf-8') as f: 
            writer = csv.writer(f) 
            for sample in sample_results: 
                rank_of_true = next((doc['rank'] for doc in sample['reranked_documents'] if doc['is_relevant']), None) 
                found_in_top_10 = rank_of_true is not None 
                top_docs = sample['reranked_documents'][:3] 
                row = [ 
                    sample['sample_id'], sample['question'][:200], sample['true_document_url'], 
                    sample['true_document_index'], rank_of_true if rank_of_true else 'Not found', 
                    found_in_top_10 
                ] 
                for i in range(3): 
                    if i < len(top_docs): 
                        row.extend([top_docs[i]['rerank_score'], top_docs[i]['document_url']]) 
                    else: 
                        row.extend(['', '']) 
                writer.writerow(row) 

    def log_rerank_results(self, sample_id: int, question: str, reranked_docs: List[Dict]) -> None: 
        """Log individual rerank results to CSV.""" 
        with open(self.rerank_logs_csv, 'a', newline='', encoding='utf-8') as f: 
            ...
            for doc in reranked_docs: 
                writer.writerow([ 
                    sample_id, question_length, rerank_timestamp, doc['original_rank'], doc['rank'], 
                    doc['retrieval_score'], doc['rerank_score'], doc['document_url'], doc['is_relevant'] 
                ]) 

    def log_comparison(self, sample_results: List[Dict]) -> None: 
        """Log comparison between retrieval and reranking for each sample to CSV.""" 
        with open(self.comparison_csv, 'a', newline='', encoding='utf-8') as f: 
            writer = csv.writer(f) 
            for sample in sample_results: 
                retrieval_rank = next((d['rank'] for d in sample['original_top10'] if d['is_relevant']), None) 
                rerank_rank = next((d['rank'] for d in sample['reranked_documents'] if d['is_relevant']), None) 
                rank_change = (retrieval_rank - rerank_rank) if (retrieval_rank is not None and rerank_rank is not None) else 0 
                retrieval_mrr = 1.0 / retrieval_rank if retrieval_rank else 0.0 
                rerank_mrr = 1.0 / rerank_rank if rerank_rank else 0.0 
                row = [ 
                    sample['sample_id'], sample['question'][:200], 
                    retrieval_rank if retrieval_rank else 'Not found', 
                    rerank_rank if rerank_rank else 'Not found', 
                    rank_change, retrieval_mrr, rerank_mrr 
                ] 
                writer.writerow(row) 
...
class RerankingMetrics: 
    ...
    @staticmethod 
    def calculate_metrics( 
        loader: RetrievalResultsLoader, 
        reranker: RerankerModel, 
        documents: List[str], 
        true_keys: List[str], 
        key2docidx: Dict[str, int], 
        k_values: List[int] = [1, 3, 5, 10], 
        csv_logger: Optional[RerankerCSVLogger] = None 
    ) -> Tuple[Dict[str, float], List[Dict]]: 
        """ 
        ...
        progress_bar = tqdm(range(n), desc="Reranking and calculating metrics") 

        for j in progress_bar: 
            question, top10 = loader.get_question_documents(j) 
            ...
            doc_indices = [d['document_index'] for d in top10] 
            texts = [documents[idx] if 0 <= idx < len(documents) else "" for idx in doc_indices] 

            start_time = time.time() 
            scores = reranker.rerank(question, texts) if texts else np.array([]) 
            end_time = time.time() 
            total_rerank_time += (end_time - start_time) 

            sorted_idx = np.argsort(-scores) if len(scores) > 0 else np.array([]) 

            reranked_docs = [] 
            for nr, sidx in enumerate(sorted_idx, 1): 
                orig_doc = top10[sidx] 
                rerank_score = float(scores[sidx]) 
                reranked_docs.append({ 
                    "rank": nr, 
                    "original_rank": orig_doc["rank"], 
                    "document_index": orig_doc["document_index"], 
                    "document_url": orig_doc["document_url"], 
                    "retrieval_score": orig_doc["similarity_score"], 
                    "rerank_score": rerank_score, 
                    "is_relevant": orig_doc["is_relevant"] 
                }) 

            if csv_logger: 
                csv_logger.log_rerank_results(j, question, reranked_docs) 

            ranks = [d['rank'] for d in reranked_docs if d['is_relevant']] 
            if ranks: 
                rank = ranks[0] 
                mrr_total += 1.0 / rank 
                for k in k_values: 
            ...
            sample_results.append({ 
                ...
                "question": question, 
                "true_document_url": true_key, 
                "true_document_index": true_doc_idx, 
        ...
        if valid_samples > 0: 
            for k in k_values: 
                metrics[f"Recall@{k}"] /= valid_samples 
            metrics["MRR"] = mrr_total / valid_samples 

        metrics["Valid_Samples"] = valid_samples 
        metrics["Total_Samples"] = n 
        metrics["avg_rerank_time_ms"] = (total_rerank_time / n * 1000) if n > 0 else 0.0 

        return metrics, sample_results 


class RerankerEvaluator: 
    """Main evaluator class that orchestrates the reranking evaluation process.""" 
    ...
    def __init__( 
        ...
        retriever_model: str = "dangvantuan/vietnamese-document-embedding" 
    ): 
        df_train = pd.read_json(train_path) 
        self.true_keys = df_train["article_url"].tolist() 

        df_articles = pd.read_json(articles_path) 
        self.documents = df_articles["document_text"].tolist() 
        self.document_urls = df_articles["article_url"].tolist() 
        self.key2docidx = {url: idx for idx, url in enumerate(self.document_urls)} 

        self.loader = RetrievalResultsLoader(top10_path) 
        self.reranker = RerankerModel(model_name) 
    ...
    def run_evaluation( 
        ...
        batch_size: int = 32, 
        k_values: List[int] = [1, 3, 5, 10], 
        output_file: str = "vietnamese_rerank_evaluation_results.json", 
        enable_csv_logging: bool = True, 
        csv_base_filename: str = "vietnamese_rerank_eval" 
    ) -> Dict[str, Union[Dict, List, str]]: 
        """Run the complete reranking evaluation pipeline.""" 
        print("Starting reranking evaluation...") 
        print(f"Number of questions: {len(self.loader)}") 

        if enable_csv_logging: 
            self.csv_logger = RerankerCSVLogger(csv_base_filename) 
            print("CSV logging enabled. Files will be saved with timestamp.") 

        print("\n1. Loading reranker model...") 
        self.reranker.load_model() 

        print("\n2. Reranking and calculating metrics...") 
        metrics, sample_results = RerankingMetrics.calculate_metrics( 
            self.loader, 
            self.reranker, 
            self.documents, 
            self.true_keys, 
    ...
    def _display_results( 
        self, model_name: str, metrics: Dict[str, float], sample_results: List[Dict], 
        output_file: str, csv_files: Dict[str, str] = {} 
    ) -> None: 
        """Display evaluation results.""" 
        print("\n" + "="*60) 
        ...
        print(f"Retriever Model: {self.retriever_model}") 
        print(f"Valid samples: {metrics['Valid_Samples']}/{metrics['Total_Samples']}") 
        ...
        if csv_files: 
            ...
            print(f" Metrics: {csv_files['metrics']}") 
            print(f" Detailed results: {csv_files['detailed']}") 
            print(f" Rerank logs: {csv_files['rerank_logs']}") 
            print(f" Comparison: {csv_files['comparison']}") 
        ...
        for i, sample in enumerate(sample_results[:3]): 
            ...
            for doc in sample['reranked_documents'][:3]: 
                status = "✓" if doc['is_relevant'] else "✗" 
                print(f" {doc['rank']}. [{status}] Score: {doc['rerank_score']:.4f} (Original rank: {doc['original_rank']})") 
                print(f" URL: {doc['document_url']}") 
